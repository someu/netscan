...
func GetServerStatusFlags(flags uint16) map[string]bool {
	...
	ret, _ := grab.ListFlagsToSet(uint64(flags), consts)
	return ret
}
...
func (e *ERRPacket) GetScanError() *grab.ScanError {
	return &grab.ScanError{
...
func (c *Connection) readPacket() (*ConnectionLogEntry, error) {
	...
	if packetSize > 0x00080000 {
		...
		status := grab.SCAN_UNKNOWN_ERROR
		if n > 1 && temp[0] == 0xff {
			// it looks like an ERRPacket: return SCAN_APPLICATION_ERROR
			status = grab.SCAN_APPLICATION_ERROR
		}
		return nil, grab.NewScanError(status, err)
	}
