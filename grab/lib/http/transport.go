...
func (t *Transport) RoundTrip(req *Request) (*Response, error) {
	...
	for {
		...
		if cm.targetScheme == "https" {
			...
			case *tls.Conn:
				// TODO/HACK: This is a local fork of the library, so it should always be a grab.TLSConnection...
				req.TLSLog = &grab.TLSLog{HandshakeLog: conn.GetHandshakeLog(), HeartbleedLog: conn.GetHeartbleedLog()}
			case *grab.TLSConnection:
