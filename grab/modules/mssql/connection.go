...
func (connection *Connection) readPreloginPacket() (*TDSPacket, *PreloginOptions, error) {
	...
	if packet.Type != TDSPacketTypeTabularResult {
		return packet, nil, &grab.ScanError{Status: grab.SCAN_APPLICATION_ERROR, Err: err}
	}
	defer grab.LogPanic("Error decoding Prelogin packet %#v", packet.Body)
