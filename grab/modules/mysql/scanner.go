// Package mysql provides the mysql implementation of the grab.Module.
...
func (s *Scanner) Scan(t grab.ScanTarget) (status grab.ScanStatus, result interface{}, thrown error) {
	var tlsConn *grab.TLSConnection
	sql := mysql.NewConnection(&mysql.Config{})
	defer func() {
		recovered := recover()
		if recovered != nil {
			thrown = recovered.(error)
			status = grab.TryGetScanStatus(thrown)
	...
	return grab.SCAN_SUCCESS, nil, nil
}
