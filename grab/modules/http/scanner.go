// Package http contains the grab Module implementation for HTTP(S).
//
...
// Module is an implementation of the grab.Module interface.
type Module struct {
}
...
// It is used to implement the grab.Scanner interface.
type scan struct {
...
func (scan *scan) dialContext(ctx context.Context, network string, addr string) (net.Conn, error) {
	dialer := grab.GetTimeoutConnectionDialer(scan.scanner.config.Timeout)
		...
		if scan.target.IP != nil && scan.target.Domain != "" {
			...
			if err != nil {
				...
				if host == scan.target.Domain {
					resolver, err := grab.NewFakeResolver(scan.target.IP.String())
					if err != nil {
						return nil, err
					}
...
// grab.GetTLSConnection()
func (scan *scan) getTLSDialer(t *grab.ScanTarget) func(net, addr string) (net.Conn, error) {
...
func (scanner *Scanner) newHTTPScan(t *grab.ScanTarget, useHTTPS bool) *scan {
	ret := scan{
		scanner: scanner,
		target:  t,
		transport: &http.Transport{
...
func (scan *scan) Grab() *grab.ScanError {
	// TODO: Allow body?
	request, err := http.NewRequest(scan.scanner.config.Method, scan.url, nil)
	if err != nil {
		return grab.NewScanError(grab.SCAN_UNKNOWN_ERROR, err)
	}
	...
	if err != nil {
		...
		case ErrTooManyRedirects:
			...
			return grab.NewScanError(grab.SCAN_APPLICATION_ERROR, err)
		default:
			return grab.DetectScanError(err)
		}
	}
...
// Scan implements the grab.Scanner interface and performs the full scan of
...
func (scanner *Scanner) Scan(t grab.ScanTarget) (grab.ScanStatus, interface{}, error) {
