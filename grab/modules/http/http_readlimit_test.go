...
func (cfg *readLimitTestConfig) getScanner(t *testing.T) *Scanner {
	...
	grab.DefaultBytesReadLimit = cfg.maxReadSize
...
type readLimitTestConfig struct {
	...
	expectedStatus grab.ScanStatus
...
var readLimitTestConfigs = map[string]*readLimitTestConfig{
	...
	"truncate_read_body": {
		...
		headerSize:     64,
		bodySize:       4096,
		expectedStatus: grab.SCAN_SUCCESS,
	},
	...
	"truncate_read_header": {
		...
		expectedStatus: grab.SCAN_UNKNOWN_ERROR,
	},
...
func (cfg *readLimitTestConfig) runTest(t *testing.T, testName string) {
	...
	if cfg.expectedStatus == grab.SCAN_SUCCESS {
		body := getBody(ret)
		if body == "" {
...
func TestReadLimitHTTP(t *testing.T) {
	if grab.DefaultReadLimitExceededAction != grab.ReadLimitExceededActionTruncate {
		t.Logf("Warning: DefaultReadLimitExceededAction is %s, not %s", grab.DefaultReadLimitExceededAction, grab.ReadLimitExceededActionTruncate)
	}
