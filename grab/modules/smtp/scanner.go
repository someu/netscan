// Package smtp provides a grab module that scans for SMTP mail
// servers.
// Default Port: 25 (TCP)
//
...
var ErrInvalidResponse = grab.NewScanError(grab.SCAN_PROTOCOL_ERROR, errors.New("Invalid response for SMTP"))
...
func (flags *Flags) Validate(args []string) error {
	if flags.StartTLS && flags.SMTPSecure {
		...
		return grab.ErrInvalidArguments
	}
	if flags.EHLODomain != "" {
		flags.SendEHLO = true
	}
...
func VerifySMTPContents(banner string) (grab.ScanStatus, int) {
	code, err := getSMTPCode(banner)
	lowerBanner := strings.ToLower(banner)
	switch {
	case err == nil && (code < 200 || code >= 300):
		return grab.SCAN_APPLICATION_ERROR, code
	case err == nil,
		...
		return grab.SCAN_SUCCESS, 0
	default:
		return grab.SCAN_PROTOCOL_ERROR, 0
	}
}
...
func (scanner *Scanner) Scan(target grab.ScanTarget) (grab.ScanStatus, interface{}, error) {
	...
	if sr == grab.SCAN_PROTOCOL_ERROR {
	...
	if scanner.config.StartTLS {
		...
		if code < 200 || code >= 300 {
			return grab.SCAN_APPLICATION_ERROR, result, fmt.Errorf("SMTP error code %d returned from STARTTLS command (%s)", code, strings.TrimSpace(ret))
		}
	...
	if sr == grab.SCAN_APPLICATION_ERROR {
