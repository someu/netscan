// Package modbus provides a grab module that scans for modbus.
...
type Flags struct {
	grab.BaseFlags
	// Protocols that support TLS should include grab.TLSFlags
...
func (scanner *Scanner) Scan(target grab.ScanTarget) (grab.ScanStatus, interface{}, error) {
	...
	if res.Function&0x7F != ModbusFunctionEncapsulatedInterface {
		...
		return grab.SCAN_PROTOCOL_ERROR, nil, fmt.Errorf("Invalid response function code 0x%02x (raw = %s)", res.Function, hex.Dump(res.Raw))
	}
	if scanner.config.Strict && (scanner.config.UnitID != 0 && res.UnitID != int(scanner.config.UnitID)) {
		...
		return grab.SCAN_PROTOCOL_ERROR, nil, fmt.Errorf("Invalid response unit ID 0x%02x (raw = %s)", res.UnitID, hex.Dump(res.Raw))
	}
	...
	if err != nil {
		...
		return grab.SCAN_PROTOCOL_ERROR, nil, err
	}

	status := grab.SCAN_SUCCESS
	if res.IsException() {
		// Note the exception, but note that the modbus protocol was detected
		status = grab.SCAN_APPLICATION_ERROR
	}
