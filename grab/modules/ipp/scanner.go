// Package ipp provides a grab module that scans for ipp.
...
func shouldReturnAttrs(length, soFar, size, upperBound int) (bool, error) {
	if soFar+length > size {
		...
		return true, grab.NewScanError(grab.SCAN_PROTOCOL_ERROR, errors.New("Reported field length runs out of bounds."))
...
func detectReadBodyError(err error) error {
	if err == io.EOF || err == io.ErrUnexpectedEOF {
		return grab.NewScanError(grab.SCAN_PROTOCOL_ERROR, errors.New("Fewer body bytes read than expected."))
	}
	return grab.NewScanError(grab.TryGetScanStatus(err), err)
}
...
func (scanner *Scanner) tryReadAttributes(resp *http.Response, scan *scan) *grab.ScanError {
	...
	if resp.StatusCode != 200 {
		return grab.NewScanError(grab.SCAN_APPLICATION_ERROR, errors.New("Response returned with status "+resp.Status))
	}
	...
	if !isIPP(resp) {
		return grab.NewScanError(grab.SCAN_PROTOCOL_ERROR, errors.New("IPP Content-Type not detected."))
	}
...
func (scanner *Scanner) augmentWithCUPSData(scan *scan, target *grab.ScanTarget, version *version) *grab.ScanError {
	...
	if versionNotSupported(scan.results.CUPSResponse.BodyText) {
		return grab.NewScanError(grab.SCAN_APPLICATION_ERROR, ErrVersionNotSupported)
	}
...
func sendIPPRequest(scan *scan, body *bytes.Buffer) (*http.Response, *grab.ScanError) {
	request, err := http.NewRequest("POST", scan.url, body)
	if err != nil {
		// TODO: Log the error to see what exactly went wrong
		return nil, grab.DetectScanError(err)
	}
	...
	if err != nil {
		...
		case ErrTooManyRedirects:
			return resp, grab.NewScanError(grab.SCAN_APPLICATION_ERROR, err)
		default:
			return resp, grab.DetectScanError(err)
		}
	}
	// TODO: Examine whether an empty response overall is a connection error; see RFC 8011 Section 4.2.5.2
	if resp == nil {
		return resp, grab.NewScanError(grab.SCAN_CONNECTION_TIMEOUT, errors.New("No HTTP response"))
	}
	...
	if resp.Body == nil {
		return resp, grab.NewScanError(grab.SCAN_PROTOCOL_ERROR, errors.New("Empty body."))
	}
	return resp, nil
}
...
func (scanner *Scanner) Grab(scan *scan, target *grab.ScanTarget, version *version) *grab.ScanError {
...
// Taken from grab http library, slightly modified to use slightly leaner scan object
func (scan *scan) getTLSDialer(scanner *Scanner) func(net, addr string) (net.Conn, error) {
	return func(net, addr string) (net.Conn, error) {
		outer, err := grab.DialTimeoutConnection(net, addr, scanner.config.BaseFlags.Timeout, 0)
		if err != nil {
			return nil, err
		}
...
// Adapted from newHTTPScan in grab http module
func (scanner *Scanner) newIPPScan(target *grab.ScanTarget, tls bool) *scan {
	newScan := scan{
	...
	transport.DialContext = grab.GetTimeoutConnectionDialer(scanner.config.Timeout).DialContext
...
func (scanner *Scanner) tryGrabForVersions(target *grab.ScanTarget, versions []version, tls bool) (*scan, *grab.ScanError) {
	scan := scanner.newIPPScan(target, tls)
	defer scan.Cleanup()
	var err *grab.ScanError
...
func (scanner *Scanner) Scan(target grab.ScanTarget) (grab.ScanStatus, interface{}, error) {
	...
	if err != nil {
		...
		if err.Err == ErrVersionNotSupported || err.Status == grab.SCAN_PROTOCOL_ERROR {
		...
		if scanner.config.TLSRetry && !scanner.config.IPPSecure {
			...
			if retryErr != nil {
				...
				return grab.TryGetScanStatus(retryErr), nil, retryErr
			}
			return grab.SCAN_SUCCESS, &retry.results, nil
		}
		if scan.shouldReportResult(scanner) {
			return err.Unpack(&scan.results)
		}
