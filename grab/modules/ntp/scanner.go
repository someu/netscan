// Package ntp provides a grab module that probes for the NTP service.
...
// Results is the struct that is returned to the grab framework from Scan()
type Results struct {
...
// Module is the grab module implementation
type Module struct {
}
...
func (scanner *Scanner) MonList(sock net.Conn, result *Results) (grab.ScanStatus, error) {
	...
	if err != nil {
		...
		case err == ErrInvalidResponse:
			...
			return grab.SCAN_PROTOCOL_ERROR, err
		case isInfoError(err):
			return grab.SCAN_APPLICATION_ERROR, err
		default:
			return grab.TryGetScanStatus(err), err
		}
	}
	return grab.SCAN_SUCCESS, err
}
