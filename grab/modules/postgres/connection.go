...
func (c *Connection) tryReadPacket(header byte) (*ServerPacket, *grab.ScanError) {
	var length [4]byte
	...
	if length[0] > 0x00 {
		...
		if n < 2 {
			return nil, grab.NewScanError(grab.SCAN_PROTOCOL_ERROR, fmt.Errorf("Server returned too little data (%d bytes: %s)", n, hex.EncodeToString(buf[:n])))
		}
		...
		return nil, grab.NewScanError(grab.SCAN_PROTOCOL_ERROR, fmt.Errorf("Server returned too much data: length = 0x%x; first %d bytes = %s", bodyLen, n, hex.EncodeToString(buf[:n])))
	}
	sizeToRead := bodyLen
...
func (c *Connection) RequestSSL() (bool, *grab.ScanError) {
	...
	if err := c.SendU32(postgresSSLRequest); err != nil {
		return false, grab.DetectScanError(err)
	}
	var header [1]byte
	...
	if header[0] < '0' || header[0] > 'z' {
		...
		return false, grab.NewScanError(grab.SCAN_PROTOCOL_ERROR, fmt.Errorf("Response message type 0x%02x was not an alphanumeric character", header[0]))
	}
	switch header[0] {
	case 'N':
		return false, nil
	...
	switch packet.Type {
	case 'E':
		return false, grab.NewScanError(grab.SCAN_APPLICATION_ERROR, fmt.Errorf("Application rejected SSLRequest packet -- response = %s", packet.ToString()))
	default:
		...
		return false, grab.NewScanError(grab.SCAN_PROTOCOL_ERROR, fmt.Errorf("Unexpected response type '%c' from server (full response = %s)", packet.Type, packet.ToString()))

	}
}

// ReadPacket reads a ServerPacket from the server.
func (c *Connection) ReadPacket() (*ServerPacket, *grab.ScanError) {
	var header [1]byte
	_, err := io.ReadFull(c.Connection, header[0:1])
	if err != nil {
		return nil, grab.DetectScanError(err)
	...
	if header[0] < '0' || header[0] > 'z' {
		// Back-end messages always start with the alphanumeric Byte1 value
		// We could further constrain this to currently-valid message types, but then we may incorrectly reject future versions
		return nil, grab.NewScanError(grab.SCAN_PROTOCOL_ERROR, fmt.Errorf("Response message type 0x%02x was not an alphanumeric character", header[0]))
	}
...
func (c *Connection) GetTLSLog() *grab.TLSLog {
	if !c.IsSSL {
		return nil
	}
	return c.Connection.(*grab.TLSConnection).GetLog()
}
...
func (c *Connection) ReadAll() ([]*ServerPacket, *grab.ScanError) {
	var ret []*ServerPacket
	for {
		response, readError := c.ReadPacket()
		if readError != nil {
			if readError.Status == grab.SCAN_IO_TIMEOUT || readError.Err == io.EOF {
				return ret, nil
			}
