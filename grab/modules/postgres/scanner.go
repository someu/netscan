// Package postgres contains the postgres grab Module implementation.
...
// Scanner is the grab scanner type for the postgres protocol
type Scanner struct {
	Config *Flags
}

// Module is the grab module for the postgres protocol
type Module struct {
}
...
func (s *Scanner) newConnection(t *grab.ScanTarget, mgr *connectionManager, nossl bool) (*Connection, *grab.ScanError) {
	var conn net.Conn
	var err error
	...
	if !nossl && !s.Config.SkipSSL {
		...
		if hasSSL {
			if err = s.DoSSL(&sql); err != nil {
				return nil, grab.NewScanError(grab.SCAN_APPLICATION_ERROR, err)
			}
			sql.IsSSL = true
		}
	}
...
func (s *Scanner) Scan(t grab.ScanTarget) (status grab.ScanStatus, result interface{}, thrown error) {
	...
	// Also do TLS handshake, if configured / supported
	{
		...
		if err := sql.SendU32(0x00); err != nil {
			return grab.TryGetScanStatus(err), &results, err
		}
	...
	// Send too-high protocol version (255.255) StartupMessage to get full error message (including line numbers, useful for probing server version)
	{
		...
		if err := sql.SendU32(0xff<<16 | 0xff); err != nil {
			...
			return grab.SCAN_PROTOCOL_ERROR, &results, err
		}
		response, readErr := sql.ReadPacket()
		if readErr != nil {
			return readErr.Unpack(&results)
