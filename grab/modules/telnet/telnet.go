...
func GetTelnetBanner(logStruct *TelnetLog, conn net.Conn, maxReadSize int) (err error) {
	...
	bannerSlice, err := grab.ReadAvailableWithOptions(conn, READ_BUFFER_LENGTH, 500*time.Millisecond, 0, maxReadSize-len(logStruct.Banner))
	if bannerSlice != nil {
	...
	if err != nil && err != io.EOF && !grab.IsTimeoutError(err) {
		return err
	}
	// Make sure it is a telnet banner
	if !logStruct.isTelnet() {
		return grab.NewScanError(grab.SCAN_PROTOCOL_ERROR, errors.New("Invalid response for Telnet"))
	}
	return nil
}
